...
**STAGE 4 HALTED — BC big (192×24, k=1) did not learn; stopped at epoch 3 per runbook §3
and returned to thread `01a0147d` for a ruling (pending Amendment 3).** Nothing is running on
the box. BC control is done (row 3). Cause, from the code: `ResidualBlock` (`model.py`) is
`gelu(x + F(x))` with no normalization and no residual scaling; 24 such blocks at 192 ch blow
up at init (epoch-1 avg_loss 36.9 vs control 2.95) and `train_bc` has no grad clipping.
`ReZeroResidualBlock` (α = 0 init, identity at start) already exists for growth blocks.
...
| 4 | BC big (192×24, k=1) | §3 | FAILED (attempt 1, plain blocks) — policy CE flat at 1.6–1.8 from step 200 through epoch 3; epoch 1/2 val top-1 44.13 % / 44.71 % (≈ majority-discard baseline); stopped by hand, guard `UNIT-EXITED` (cgroup peak 28.25 GiB); artifacts kept in `bc-big/` | `best_epoch`, val CE, top-1 (zeroed events) overall + per seat, `best.pt` sha256 |
