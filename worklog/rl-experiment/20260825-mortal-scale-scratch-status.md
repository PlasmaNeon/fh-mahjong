...
| 100 | **−0.2375** | ±0.0721 (sig. YES) | 0.0708 / 0.0458 | yes | kill iff `delta100 − delta75 ≤ 0` and `delta100 < −0.20` — **slope +0.0528 > 0, so the rule does NOT fire**; lap was eligible to continue. placement +0.2236; 4th-share Δ +0.0563, training-utility Δ −0.2001; deal-in 0.1014 vs 0.1033. **large_loss 0.1333 → 0.0708, its Δ CI now straddles zero [−0.0033, +0.0533]** — first secondary to reach parity with the anchor. Tail gate still FAIL. Intervals: +0.0542, +0.0805, +0.0528; reaching −0.0600 needed +0.0444 per 25 over the remaining 100 iters, under all three |
| 125 | | | | | |
| 150 | | | | | |
| 175 | | | | | |
...
- `2026-09-09 23:47 UTC` — mortal-scale-scratch — §5 control lap **RESUMED** (`msscratch-control`, `--resume-from-state control/ckpt/train_state.pt`, both guards retargeted and armed). Box had rebooted; `/root/fh-mahjong` still `8ad2688` clean and the bridge still `a487bcb7…` byte-identical, so screens 125–200 stay comparable to 25–100. State was saved at iteration 100, so the resume re-ran and rewrote `iter_101.pt`. Remaining: ~99 iterations, ~14 h.
- `2026-09-09` — mortal-scale-scratch — **liveness note:** collection workers spawn as `python3 -c from multiprocessing.spawn import spawn_main`, so they do NOT match a `pgrep -f "fh-mj|fh_mahjong"` pattern, and the parent sits idle in `queue.get()` for the whole ~8.5-min collection with near-zero CPU. Judge a lap by the unit state plus a rising `iter_*.pt` count, never by pgrep or parent CPU.
- `—` — (add next event here)
