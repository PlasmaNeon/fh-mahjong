# Chongci IQL-era experiment ledger (archive, 2026-03 to 2026-06)

Roughly a hundred experiments from the offline-IQL line, overwhelmingly negative results.
Kept as a map of paths already walked: **search here before proposing any risk head,
auxiliary target, counterfactual supervision scheme, or first-divergence replay
objective.** Current state is in
[`chongci-rl-experiment-progress.md`](./chongci-rl-experiment-progress.md).

Two rules came out of this era:

1. **An auxiliary or risk head is unusable until it is calibrated.** Require AUC above
   random, monotonic risk bands, and acceptable severity error before spending evaluation
   budget or wiring anything into serving. Three successive input and loss designs here
   landed at chance ranking (large-loss AUC 0.4998, 0.5096, 0.4990); coefficient sweeps
   never moved it.
2. **Evaluation reliability comes before any result is believable.** Per-hand wall seeds
   and same-priority interrupt tie-breaking are what made results reproducible; see
   "Evaluation Stability Issue".

Navigation note: most entries sit under `## Recommended Next Experiments`, which covers
completed work rather than proposed work.
