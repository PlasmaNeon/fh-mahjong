...
**Spec:** `worklog/specs/2026-07-14-eval-hygiene-obsfix-design.md` (approved). Branch: `claude/eval-hygiene` (exists, off main @ 97a84c9).
...
**Files:**
- Modify: `worklog/rl-experiment/chongci-rl-experiment-progress.md` (append a subsection to the maintenance/protocol section at the top of the file, after the existing protocol rules)
...
Read the protocol/maintenance section at the top of `worklog/rl-experiment/chongci-rl-experiment-progress.md` and append this subsection to it (adjust the heading level to match neighbors):
...
```bash
git add worklog/rl-experiment/chongci-rl-experiment-progress.md internal/rl/AGENTS.md ai/AGENTS.md
...
- [ ] **Step 4: Record** — append the paired result AND the measured `cluster_design_effect` of the screening window to `worklog/rl-experiment/chongci-rl-experiment-progress.md` (it calibrates Spec B's run sizes); commit the note to main per the progress-note convention.
