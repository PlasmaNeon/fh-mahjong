...
type DecisionContext struct {
    ...
    Events          []engine.PublicEvent  // snapshot COPY of game.PublicEvents()
    EventWindow     uint32                // 0 = event-free policy
    ContractVersion uint32                // EventContractV1
}
...
- `HTTPPolicy` implements `ContextPolicy`; its legacy `ChooseAction`
  remains (delegates with an empty context, window 0) for compatibility.
...
**`/act` payload (additive, backward compatible):** gains
`event_history` (uint32 array, tail-windowed, ≤ window), `event_count`,
`event_window`, `contract_version`. The currently-deployed old-champion
...
- event model + missing/short/mismatched fields or wrong contract version
  → HTTP 400 with a reason, NEVER a silent zero-history inference;
...
(B2b round-4 guard) is replaced by reconstruction from the checkpoint's
pinned `metadata.b2b` block ({event_window, privileged_critic, aux_heads,
residual_blocks}); B2b-module keys WITHOUT the metadata block still fail
closed. `CheckpointPolicy` threads events into every forward
...
2. **End-to-end action parity**: new CLI `fh-mj-serving-parity` — drives
   seeded bridge states, computes the greedy action via (a) the serving
   stack (CheckpointPolicy fed by the /act-shaped payload built from the
   DecisionContext encoding) and (b) the eval stack (TorchGreedyPolicy on
   the bridge observation), asserting exact feature equality and exact
   greedy action IDs; exact logits on the same hardware, tight tolerance
   (1e-4) + identical actions across CPU/GPU. Zero fallbacks tolerated.
   Runs in CI on mock/CPU and in the runbook against the real prod image
   with iter_075.
...
(0 for the old champion → byte-identical behavior). The review HTTP
client sends the same enriched /act payload.

## 5. Shadow mode (code in B2c, activation post-merge)

`bot.ShadowPolicy` wrapper: `{Primary bot.Policy/ContextPolicy, Shadow
ContextPolicy}`. The primary's action is returned to the game; the SAME
DecisionContext is handed to the shadow asynchronously (bounded queue,
drop-on-backpressure — the shadow must never block or affect play), and
