...
- **main.go** — Exports the bridge surface:
  ...
  - `FHSearchPoolNew` / `FHSearchPoolStep` / `FHSearchPoolClose` — test-time search exports (own handle registry, same conventions as the env-pool trio). `FHSearchPoolNew` takes a live env handle plus a `SearchPoolNewRequest` (clones/seed/max_rollout_decisions) and wraps `rl.NewSearchPool`; `FHSearchPoolStep` reuses `EnvPoolStepRequest`/`EnvPoolStepResponse` and wraps `(*rl.SearchPool).Step`.
