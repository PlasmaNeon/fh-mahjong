...
- **src/** — Application source code (pages, components, contexts, hooks, utils, proto bindings). `src/components/` holds the shared "Tabletop Glass" theme primitives used by the game-flow pages; the two tool pages (Calc/Shanten) instead use the dedicated "ledger" theme in `src/pages/ledger-theme.css`.
