...
- The seat bundle (`.seat-bundle`, `SeatBundle.tsx`) is a fixed-width box that pins the concealed hand left and the exposed stack (flowers above melds) right via `justify-content: space-between`. The concealed hand MUST keep a fixed reserved width (max 14 tiles, sized per seat: `--tile-width` for self incl. `--drawn-gap`, `--tile-small-width` for opponents) — never `width: auto`. A content-sized hand grows/shrinks by one tile on every draw/discard, and once the bundle's content overflows its span the `space-between` degrades to `flex-start`, so the exposed stack (the flowers) jumps sideways each turn — the "shaking flowers" bug. Fixed reserved width keeps the hand footprint constant so the flowers never move on a draw.
