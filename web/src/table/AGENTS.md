...
- **roundResult.css** — Fenghua settlement-ledger styling for the shared result dialog:
  ...
  - Keeps all four seat deltas in one compact settlement strip on phones; live readiness is an explicit `Ready` / `Waiting` dot-and-label row, while replay payouts omit status when readiness data is unavailable
...
- The seat bundle (`.seat-bundle`, `SeatBundle.tsx`) is a fixed-width box that pins the concealed hand left and the exposed stack (flowers above melds) right via `justify-content: space-between`. The concealed hand MUST keep a fixed reserved width (never `width: auto`): a content-sized hand grows/shrinks by one tile on every draw/discard, and once the bundle's content overflows its span the `space-between` degrades to `flex-start`, so the exposed stack (the flowers) jumps sideways each turn — the "shaking flowers" bug. But the reservation MUST track the hand's real max size for its meld count, NOT a blanket 14 tiles. `SeatBundle.tsx` sets `--seat-hand-tiles` = `concealedHandReserveTiles(meldCount)` = `max(2, 14 - 3*melds)` (engine invariant: concealed-with-draw + 3*melds == 14), and the CSS widths (`--tile-width` for self incl. `--drawn-gap`, `--tile-small-width` for opponents) multiply by it. Over-reserving a flat 14 tiles for a hand that has *called* melds (only a few concealed tiles left) overflows the span, and because the bundle is centred on its pivot the overflow shoves the exposed melds outward — the first-formed meld spilled past the table edge ("first meld pushed out"). Meld count is constant across a draw/discard, so the reservation still keeps the flowers from shaking.
...
- Keep the payout strip at four equal columns on phone layouts. Its shared outer border, internal dividers, and compact status rows are intentional vertical-density controls; do not restore the old phone-only 2x2 card grid or a visible `Payouts` heading.
