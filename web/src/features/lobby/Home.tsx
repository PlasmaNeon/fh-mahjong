        ...
        <main className="ledger-page">
            <div className="club-home">
                <section>
                    ...
                    <p className="club-home__lead">
                        A late-night table for Fenghua rules—find a live match, open a private room,
                        or work through a hand at the scoring desk.
                    </p>
                </section>
                ...
                <nav className="club-home__menu" aria-label="Club menu">
                    ...
                    <div className="club-home__links">
                        ...
                        <HomeLink to={username ? '/account' : '/login'} title="Profile" detail={username ? `Signed in as ${username}` : 'Sign in or create an account'} mark="YOU" />
                    </div>
                </nav>
            </div>
        </main>
...
function HomeLink({ to, title, detail, mark, primary = false }: { to: string; title: string; detail: string; mark: string; primary?: boolean }) {
    return (
        <Link className={`club-home__link${primary ? ' club-home__link--primary' : ''}`} to={to}>
            <span><strong>{title}</strong><small>{detail}</small></span>
            <span className="club-home__link-mark">{mark} →</span>
        </Link>
    )
}
