...
- **queue.go** — `InMemoryQueue`, the mutex-guarded in-process FIFO the matchmaker holds waiting players in. Split out of `matchmaker.go`; its methods are `Push`/`PushUnique`/`Items`/`Len`/`PopN` (they were `RPush`/`LRange`/`LLen`/`LPopCount` until the Redis vocabulary was dropped — Redis itself went in PR #129).
