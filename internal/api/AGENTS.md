...
- **server.go** — Gin HTTP server setup and route registration:
  ...
  - Public password recovery, **not linked from the frontend**: `POST /api/v1/auth/password-reset/request` (always 204, never discloses whether an account or address exists) and `POST /api/v1/auth/password-reset/confirm` (one generic 400 for every failure — the sole exception: a new password failing `min=8` returns the binding error, which discloses nothing about the account; success clears every *stored* session for that user — but a WebSocket connection authenticates once at upgrade and is not itself evicted, so an already-open connection for that user stays live until it disconnects on its own. Hub-level eviction of live connections by user id would be required before this flow is ever linked from the UI). The configured `mail.Sender` is `LogSender` in development (writes codes to the server log so the flow is exercisable end to end) and `mail.SuppressedSender` in production (`isProduction()`, in auth.go) — it discards the code and logs only that one was requested, since a live reset code in a production log is a worse exposure than a dormant endpoint, and with no real provider configured the code could never reach the user anyway. Swap in a real provider (and remove the suppression) before exposing any UI
...
- **auth.go** — Username + password auth backed by opaque revocable sessions. Registration takes a username and password only; email is an optional profile field (`PATCH /users/me` accepts a string to set it or `""` to clear it, and either way requires the current password; the format check also rejects anything over the column's 255-char limit before it can reach Postgres as an unhandled "value too long" 500). Changing or clearing the email retires every outstanding password-reset code for that user in the same transaction as the update — a code already in flight went to the old address, and leaving it live would let whoever still controls that inbox complete a reset the owner just acted to prevent. `isProduction()` (env-gated on `APP_ENV`/`GIN_MODE`) is the general production check; `isProductionCookie()` is a thin wrapper kept for the Secure-cookie call site. Login/register/session return `{user, csrfToken}` and set an HttpOnly cookie; no credential is serialized in JSON or stored by frontend JavaScript
- **password_reset.go** — Code-based recovery: `lookupUserByIdentifier` (shared with login), 6-digit codes bcrypt-hashed at rest with a 15-minute TTL and a 5-attempt cap, superseding any outstanding code. Bcrypt rather than a fast digest because a 6-digit code holds only ~20 bits of entropy. The attempt is reserved with an atomic, conditional DB update (`attempts < cap AND consumed_at IS NULL AND expires_at > now`) *before* the bcrypt compare, not after — bcrypt sits inside the check-then-act window for ~100ms, so a post-comparison increment would let concurrent guesses all read the same stale attempt count and blow past the cap. A successful reset (and a profile email change/clear, in `auth.go`) consumes every unconsumed code for the user, not just the one looked up, since the issue path's supersede-then-insert has no row lock. Old consumed/expired rows are pruned opportunistically on each new issue, mirroring `createSession`'s session pruning
