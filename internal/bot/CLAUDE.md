...
- **policy.go** — the `Policy` interface (the package's core contract, moved here from heuristic.go), plus `DecisionContext{State, Seat, DecisionIndex, Events}` and the additive `ContextPolicy` interface (`ChooseActionCtx`). Room dispatch (`internal/api/room_bot.go`) prefers `ChooseActionCtx` when a policy implements it, falling back to legacy `ChooseAction` otherwise. `Events` is always a snapshot copy — callers may hold it past the room's lock.
