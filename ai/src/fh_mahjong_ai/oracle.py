"""Oracle-guiding helpers: build a perfect-information policy warm-started from
the 39-channel anchor, collect oracle and feature-dropout self-play rollouts,
and run the two oracle PPO loops (`train_oracle`, `train_selfplay_oracle`).

B2b model surgery and the `train_b2b` loop live in `train_b2b.py`; the
crash-resume / `train_state.pt` machinery lives in `train_state.py`."""
