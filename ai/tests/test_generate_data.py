...
def test_seed_mod_4_rule_keeps_exactly_one_seat_per_episode(tmp_path: Path) -> None:
    ...
    # max_steps_per_episode must be large enough that MockMahjongBridge takes
    # several steps and rotates current_seat through 0..3 before terminating
    # (default is 0, which ends each mock episode after a single seat-0
    # transition and would make this test vacuous — every non-zero-mod-4
    # episode would just vanish without ever exercising cross-seat exclusion).
    stats = generate_dataset(
        episodes=8,
    ...
    assert set(by_episode.keys()) == set(range(8))  # every episode survived the filter
